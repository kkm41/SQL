...
values ('one', '111','김나리','133-110','서울시 성동구 성수동1가','1번지21호' , '017-777-7777','acc@abc.com');
insert into member(userid , pwd, name, zip_num, address1, address2, phone,email)
values ('two', '222','김길동','130-120','서울시 송파구 잠실2동','리센츠 아프트 201-505' , '017-222-333','abb@abc.com');
...
-- useid: one 사용자가 1번 상품 1개를 카트에 추가
insert into cart(userid, pseq, quantity) values('two',3,1);


-- orders와 ored_detail
-- orders와 ored_detail 추가
insert into orders(userid) value ('one');
insert into order_detail(oseq, pseq, quantity) value (1,2,1);
insert into order_detail(oseq, pseq, quantity) value (1,3,2);


insert into orders(userid) value ('two');
insert into order_detail(oseq, pseq, quantity) value (2,4,1);
insert into order_detail(oseq, pseq, quantity) value (2,5,2);

insert into orders(userid) value ('one');
insert into order_detail(oseq, pseq, quantity) value (3,3,1);
insert into order_detail(oseq, pseq, quantity) value (3,2,1);
...
-- cart 안의 pseq로 상품이름과 그리고 uesid 로 사용자 이름을 함꼐 조회하는 view를 생성합니다.
...
select * from order_detail where oseq = 10;
update order_detail set result = '2' where oseq = 10;


--관리자 입력
insert into admins values('admin','admin','관리자','010-7777-7777');
insert into admins values('scott','1234','홍길동','010-1111-2222');
