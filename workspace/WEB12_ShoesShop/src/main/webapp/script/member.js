...
function go_next(){
	//자바스크립트에서 Html 태그에 접근할때, 둘의 name 값이 같으면, 그둘은 배열로 처리합니다.
	//document.contractFr.okon 이두개이므로
	//하나는 document.contractFrm.okon[0]
	//다른 하나는 document.contractFrm.okon[1] 으로 입식합니다.
...
function go_save(){
	...
	document.joinForm.pwd.focus();
	} else if(document.joinForm.name1.value == "") {
	alert("이름을 입력해 주세요.");
	document.joinForm.name1.focus();
