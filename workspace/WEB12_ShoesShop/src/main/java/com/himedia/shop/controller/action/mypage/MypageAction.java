...
public class MypageAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(mvo == null ) response.sendRedirect("shop.do?command=loginForm");
			...
			//현재 로그인 한 사용자의 배송전 주문내역을 표시합니다.
			//한번에 2~3개의 상품을 3회에 걸쳐 주문했다면
			//진행중인 주문 내역은 3행이 표시 됩니다. ( 주문 번호별(oseq) 표시)
			
			//그리고 그들에게는 상세보기 메뉴가 있어서 그 주문에 어떤 상품이 주문 된건지 상세하게 조회할 수 있습니다.
			
			//1. mypage.jsp에 최종 전달되어서 화면에 보여질 리스트를 생성합니다.
			ArrayList<OrderVO> finalList = new ArrayList<OrderVO>();
			//finalList.get(0).getName() - > "XXXX외 2건"와 같이 구성됩니다.
			
			//order_view 에서 로그인 유저의 아이디로 아직 발송전 result가 1인것만 검색하면
			//4 4 4 5 5 6 6 <-조회된 주문번호(oseq)들
			//나에게 지금 필요한건 4 5 6
			//위와 같은 결과를 얻으려면 sql문에 distinct 키워드를 써서 조회해 옵니다.
			//select distinct oseq from order_view where userid = ? and result = '1'
			
			//scott이 주문하고 진행중인 (result = '1') 주문의 oseq 를 중복되지 않게 조회
			OrderDao odao = OrderDao.getInstance();
			ArrayList<Integer> oseqList = odao.selectOseqOrderIng(mvo.getUserid());
			
			//주문번호들로 반복실행하면 주문 상세내역을 조회 -> 조회된 내역을 하나로 합쳐서 finalList 에 담습니다.
			for(Integer oseq : oseqList ) {
				//과정: 주문버호로 주문목록을 조회(order_view)에서 조회
				//4번 주문의 2개의 상품에서 첫번째 상품의 상품이름을 "상품이름 포함 1건" 으로 변경
				//금액은 2개의 가격을 합산한 금액을 4번 상품의 price2에 저장
				//그 변경된 첫번째 상품을 finalList에 저장
				ArrayList<OrderVO> oredListByOseq = odao.selectOrderByOseq(oseq);
				
				//주문 상품 상세 리스트의 첫번째 상품을 별도의 변수에 저장
				OrderVO temp = (OrderVO) oredListByOseq.get(0);
				
				//temp의 상품 명과 가격을 수정
				temp.setPname(temp.getPname()+"포함"+oredListByOseq.size()+"건");
				
				//리스트를 이용해서 총금액을 계산
				int totalPrice =0;
				for(OrderVO ovo : oredListByOseq) {
					totalPrice += ovo.getPrice2() * ovo.getQuantity();
				}
				
				//가격 수정
				temp.setPrice2(totalPrice);
				//temp 를 finalList에 추가
				finalList.add(temp);
			}
