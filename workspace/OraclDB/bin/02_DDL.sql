--02.DDL.sql
--DDL(Datebase Definition Language)데이터 정의어
--테이블 생성 CREATE
...
--		기본키로 결함을 없애는 것. 결함없이 유지되는 것을 "개체 무결성"이라고 한다.
-- 예) 생년월일, 교유번호, 학번 등등
...
--6. 필드명 뒤에 DATATYPE은 반드시 지정하고 []안에 내용은 해당내용이 있을 때 작성하며 생략 가능
...
--13. 컬럼(필드)과 컬럼(필드)의 구분은 콤마로 하되, 마지막 컬럼이나 consraint 를 작성후엔 콤마를 찍지 않음
...
-- 예를 들면 booklist 에 존재하지 않은 도서의 번호가 rentlist의 빌려간 도서의 등록되지 못하게 하는 것

-- 테이블 생성 3
--테이블 이름 : rentlist
-- 필드 : idx(number(3)), rent_date(date), bnum(varchar2(5)),
--			mnum(varchar(5)),discount(number(4))
...
--constraint rent_pk primary key(rent_date,rent_seq) --idx같은게 없을때 이렇게도 가능
...
--외래키의 참조 대상이 되는 필드는 그쪽 테이블의 기본키이어야 한다
