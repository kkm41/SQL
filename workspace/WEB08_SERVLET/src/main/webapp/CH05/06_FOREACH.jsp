...
<body>
	<%
		...
		//배열을 만들고 만든 배열을 request의 Attribute에 저장합니다.
		request.setAttribute("mList", movieList);
		//서버에서 JSP 파일로 전달되는 배열이나 ArrayList는 보통 request의 Attribute에 담겨옵니다.
		//이를 바로 반복실행문에 적용하는 형식이 주로 사용됩니다.
		//그래서 배열을 만들고 만든 배열을 request 에 저장했습니다.
	...
	%>
	<br><br><hr><br><br>
	<!-- JSTL 과 EL 그리고 배열을 이용한 반복실행 -->
	<c:forEach items="${mList}" var ="s">
		<h2>${s}</h2>
	</c:forEach>
	<!-- 
		items = "" : 반복실행에 이용할 리스트 또는 배열을 지정(request나 session에 저장된 키값)
 	 ...
 	 <!-- 
 	 	...
 	 	status : 현재 반복순소 객체 변수
 	 	${status.count} : 1부터 시작한 반복의 현재 아이템
 	 	${status.index} : 0 부터 시작한 반복의 현재 아이템
 	  -->
 	  <br><br><hr><br><br>
 	  <table border="1" style="width:700px; text-align:center">
 	 	<tr><th>index</th><th>count</th><th>title</th></tr>
 	...
 	<br><br><hr><br><br><br><br><br>
 	 <!-- 
 	 	${state.first} : 현재 반복이 첫번째이면 true 리턴
 	 	${state.last} : 현재 반복이 마지막이면 true 리턴
 	  -->
 	  <br><br>
 	  <!-- 마지막 아이템 만 빼고 각 아이템 사이에 (,) 컴마를 찍어 출력하고 싶을 때 -->
 	  <c:forEach items = "${mList}" var="movie" varStatus ="state">
 	  	<%-- ${movie}--%> ${state.current}
